...
function EditService() {
    ...
    const [description, setDescription] = useState('');
     ...
     const fetchData = async () => {
       try {
         const response = await axios.get(`http://localhost:5000/api/service/ServiceById/${id}`);
      ...
      const handleSubmit = async (event) => {
        ...
        try {
          const response = await axios.put(`http://localhost:5000/api/service/editService/${id}`, formData, {
            headers: {
